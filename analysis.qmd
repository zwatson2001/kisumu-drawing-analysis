...
``` {r read in demographic data}
demographic_data <- read_csv(here("demo_data/drawing_recognition_demo_data.csv")); 
```

``` {r read in tracing demographic data}
tracing_demographic_data_files <- list.files((here("demo_data/tracing")), pattern = "\\.csv$", full.names = TRUE)
tracing_demographic_data <- read_csv(tracing_demographic_data_files)
```
...
``` {r filter demographic data}
participant_ids <- data |>
  select(prolific_pid) |>
  unique()

demographic_data_filtered <- demographic_data |>
  filter(`Participant id` %in% participant_ids$prolific_pid)

demographic_data_excluded <- demographic_data |>
  filter(!(`Participant id` %in% participant_ids$prolific_pid))

write_csv(demographic_data_filtered, here("output/drawing-recognition-demo-data-filtered.csv"))
write_csv(demographic_data_excluded, here("output/drawing-recognition-excluded-participants.csv"))
```
...
``` {r filter tracing demographic data}
participant_ids_tracing <- tracing_data |>
  select(prolific_pid) |>
  unique()

tracing_demographic_data_filtered <- tracing_demographic_data |>
  filter(`Participant id` %in% participant_ids_tracing$prolific_pid)

tracing_demographic_data_excluded <- tracing_demographic_data |>
  filter(!(`Participant id` %in% participant_ids_tracing$prolific_pid))

write_csv(tracing_demographic_data_filtered, here("output/tracing-demo-data-filtered.csv"))
write_csv(tracing_demographic_data_excluded, here("output/tracing-excluded-participants.csv"))
```
